...
def generate_segmentation_and_train_list(root, line_txt, names, file_name='train_gt.txt', json_name='edge_anno_cache.json'):
    """
    ...
    for i in tqdm.tqdm(range(len(line_txt))):
        ...
        fPath = os.path.join(root,names[i])
        print(fPath)
        ...
        all_points = np.zeros((10,65,2), dtype=np.float64)
        the_anno_row_anchor = np.array(list(range(550, 1200, 10)))
...
if __name__ == "__main__":
    ...
    cur_Root = "/home/wukong/Desktop/LaneDetection/EdgeDetectSet"

    names, line_txt = get_curvelanes_list(cur_Root,  'train')
    # generate training list for training
    #generate_segmentation_and_train_list(os.path.join(cur_Root, 'train'), line_txt, names)


    # names, line_txt = get_curvelanes_list(args.root,  'valid')
    #generate_segmentation_and_train_list(os.path.join(cur_Root, 'valid'), line_txt, names, file_name='valid_gt.txt', json_name='edge_anno_cache_val.json')
